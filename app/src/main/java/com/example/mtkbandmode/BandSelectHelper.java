 ...
 *   bypass that check. The only reliable path without a platform signature is:
 *     su -c "service call phone <TXN> ..."
 *   This executes as UID 0 (root), which always passes the check.
 *
 * SERVICE CALL TRANSACTION CODES (AOSP ITelephony.aidl):
 *   setAllowedNetworkTypesForReasons = 84  (Android 13+)
 *   setPreferredNetworkTypeBitmap    = 80  (Android 9-12)
 *   getAllowedNetworkTypesForReasons  = 85  (Android 13+)
 *   getPreferredNetworkTypeBitmap    = 81  (Android 9-12)
 *
 * NOTE: Transaction codes can differ by OEM ROM. If SET fails, verify with:
...
public class BandSelectHelper {
    ...
    public void applyNetworkTypeMask(long mask, Callback callback) {
        mExecutor.execute(() -> {
            ...
            // Try Android 13+ path first, fall back to 9-12 path
            String result = runRootCommand(buildSetAllowedCmd(subId, mask));
            if (result == null) {
